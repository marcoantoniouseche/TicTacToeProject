...
def catch(number):
    try:
        ...
        try:
            final_value = int(input('\tEscribe un valor correcto(Entero):\n\t'))
        except ValueError:
...
def xoro(letter):
    while not(letter == 'X' or letter == 'x' or letter == 'O' or letter == 'o'):
        letter = input('\tEscribe un valor correcto(\'X\' o \'O\'):\n\t')
    return letter
...
while play == 1:
    ...
    \t   1   2   3
    ''')
    print('\tAdvertencia: Si escribe un valor incorrecto, pierde el turno p. Ej. 14\n\tAdvertencia: Si escribe un valor que alguien escribió antes, p. Ej. 12')
    ...
    print('Si quieres jugar otra vez presiona 1 de lo contrario 0')
    play = int(input())
    while play < 0 or play > 1:
        play = catch(input('Escriba un valor correcto(0-1)\n'))
